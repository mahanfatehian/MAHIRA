...
class GrammarCardWidget(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        self.prompt_sub = QLabel("Fill the missing form, then rate your recall.")
        ...
        tips_frame = self._section_frame()
        tips_grid = QGridLayout(tips_frame)
        tips_grid.setContentsMargins(14, 14, 14, 14)
        tips_grid.setHorizontalSpacing(10)
        tips_grid.setVerticalSpacing(10)

        self.meaning_title = self._section_label("Meaning")
        self.meaning_label = self._value_box()
        self.btn_reveal_meaning = self._secondary_btn("Reveal")
        self.btn_reveal_meaning.setFixedWidth(100)

        self.hint_title = self._section_label("Hint")
        self.hint_label = self._value_box()
        self.btn_reveal_hint = self._secondary_btn("Reveal")
        self.btn_reveal_hint.setFixedWidth(100)

        self.grammar_tip_title = self._section_label("Grammar tip")
        self.grammar_tip_label = self._value_box()
        self.btn_reveal_grammar = self._secondary_btn("Reveal")
        ...
        answer_frame = self._section_frame()
        answer_layout = QVBoxLayout(answer_frame)
        answer_layout.setContentsMargins(14, 14, 14, 14)
        ...
        self.btn_check = self._primary_btn("Check")
        self.btn_skip = self._skip_btn("Skip")
        ...
        answer_layout.addLayout(input_row)
        ...
        rate_row = QHBoxLayout(rating_frame)
        rate_row.setContentsMargins(14, 14, 14, 14)
        ...
        root.addWidget(self.prompt_sub)
        root.addWidget(sep)
    ...
    @staticmethod
    def _skip_btn(text: str) -> QPushButton:
        b = QPushButton(text)
        b.setStyleSheet(
            """
            QPushButton {
    ...
    def reset_for_next(self) -> None:
        ...
        for _, b in self._rating_buttons:
            b.setEnabled(False)
            b.setStyleSheet(self._rating_style(_, recommended=False))
